# -*- coding: utf-8 -*-
"""DataPreparationLLMs_py.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1tjmZI9BmI7bKZH6utOVtlydyb7rQxezn
...
def generate_model_parameters(filename):
    ...
    column_types = {
        "Parameter": str,
        "Value": lambda x: str(x) if isinstance(x, str) else (int(x) if isinstance(x, int) else float(x))
    }
    ...
    # Write to CSV with specified data types
    with open(filename, mode='w', newline='', encoding='utf-8') as file:
        writer = csv.writer(file)
        ...
        for row in rows:
            parameter = row[0]
            value = row[1]
            value_type = column_types["Value"](value) if parameter != "Parameter" else column_types["Parameter"]
            writer.writerow([parameter, value_type])
...
def generate_training_results(filename, training_loss, train_runtime, train_samples_per_second, train_steps_per_second):
    ...
    # Prepare the data rows for training results
    rows = [
        ["Training Loss", training_loss],
        ["Train Runtime", train_runtime],
        ["Train Samples per Second", train_samples_per_second],
    ...
    with open(filename, mode='w', newline='', encoding='utf-8') as file:
        ...
        for row in rows:
            metric = row[0]
            value = row[1]
            value_type = column_types["Value"](value)
            writer.writerow([metric, value_type])
...
def generate_model_details(filename, model_name, model_size, tensor_type, languages_supported, base_model):
    ...
    rows = [
        {"Attribute": "Model Name", "Value": model_name},
        {"Attribute": "Model Size", "Value": model_size},
        {"Attribute": "Tensor Type", "Value": tensor_type},
        {"Attribute": "Languages Supported", "Value": languages_supported},
        {"Attribute": "Base Model", "Value": base_model}
    ]
    ...
    with open(filename, mode='w', newline='', encoding='utf-8') as file:
        ...
        writer.writeheader()
        for row in rows:
            row["Value"] = column_types["Value"](row["Value"])
            writer.writerow(row)
